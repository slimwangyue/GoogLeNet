...
#                                                           gpu_id=0).astype('float32')
    ...
    if filters_size == 3:
        plot(weight_vector, labels_all[name], centers_all[name], name)
...
def plot(vector, labels, centers, name):
	...
	for v, l in zip(vector, labels):
		ax.scatter(v[0], v[1], v[2], s=0.001, color=colors[l])



	plt.savefig('./plots/' + name + '.png', figsize=(50, 40), dpi=1000)
