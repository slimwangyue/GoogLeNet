...
def run_gpu_eval(use_compression=False, use_quantization=False, compute_energy=False, use_pretrained_model=True, epoch_num=0):
    ...
    with tf.Graph().as_default():
        with tf.Session(config=config) as sess:
            ...
            alpha = FLAGS.alpha
            memory = 0
            k_list, memory_all = get_k_list(alpha)
            memory_weight = 0
            memory_weight_log = 0
            ...
            for v in tf.trainable_variables() + bn_moving_vars:
                if 'weights' in v.name:
                    ...
                    if use_compression:
                        ...
                        weight_clustered, mse, huffman_length = cluster_conv(weights, k, seed, v.name, epoch_num)
            ...
            print('**********************************')
            print('Compression Rate is: {:.4f}'.format(memory_all / memory_weight))
            print('**********************************')
...
def plot(vector, labels, centers, name, epoch, n_clusters, max_prob):

	num_plots = centers.shape[0]
	marker = [',', '+', '*', '<', '>', '^', 'v']
	fig = plt.figure()
	ax = Axes3D(fig)
	...
	for v, l in zip(vector, labels):
		ax.scatter(v[0], v[1], v[2], s=0.00001, color=colors[l], marker=marker[l % 7])



	plt.savefig('./plots/' + name.split('/')[1] + '; epoch:' + str(epoch) + '; cluster:' + str(n_clusters) + '; max_prob:' + str(max_prob) + '.png', figsize=(100, 80), dpi=2000)
